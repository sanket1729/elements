...
from test_framework.script import CScript, OP_1, OP_EQUAL, OP_SHA256FINALIZE, OP_SHA256INITIALIZE, OP_SHA256UPDATE, TaprootSignatureHash, taproot_construct, SIGHASH_DEFAULT, SIGHASH_ALL, SIGHASH_NONE, SIGHASH_SINGLE, SIGHASH_ANYONECANPAY
...
class TapHashPeginTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for k in range(0, 81, 2):
            ...
            self.tapscript_satisfy_test(CScript([OP_SHA256INITIALIZE, OP_SHA256FINALIZE, hash, OP_EQUAL]), inputs = [bytes(0), preimage])
        ...
        for pre_len in [64*i for i in range(2, 10)] + [100, 520, 521, 800, 1000, 10000]:
            ...
            script = CScript([OP_SHA256INITIALIZE] + [OP_SHA256UPDATE]*(len(wit)-2) + [OP_SHA256FINALIZE, hash, OP_EQUAL])
